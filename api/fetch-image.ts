    ...
    if (!accessToken) {
      ...
      if (base64Creds) {
        ...
        return res.status(500).json({
          error:
            "Credenciais ausentes. Defina GOOGLE_APPLICATION_CREDENTIALS_BASE64 ou GOOGLE_APPLICATION_CREDENTIALS_JSON",
        });
      }
      ...
      const fetched = await (client as any).getAccessToken();
      accessToken =
